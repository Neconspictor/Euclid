...
namespace nex
{
	...
	static int solve_depressed_cubic(const complex_t* poly, complex_t* results) {
		...
		cubic_root_of_unity.re = -0.5;
		cubic_root_of_unity.im = 0.5 * sqrt(3.0);
		for (i = 0; i < 3; ++i) {
			if (i == 0)
	...
	static complex_t complex_inverse(complex_t x) {
		...
		complex_t result;
		result.re = x.re / sqr_norm;
		result.im = -x.im / sqr_norm;
		return result;
	}
	...
	static complex_t complex_sqrt(complex_t x) {
		...
		const double t = sqrt(0.5 * (r - x.re));
		result.im = x.im >= 0 ? t : -t;
		return result;
	}

	static complex_t complex_pow_real(complex_t x, double power) {
		const double norm = sqrt(pow(complex_sqr_norm(x), power));
		const double arg = atan2(x.im, x.re) * power;
		complex_t result;
		result.re = norm * cos(arg);
		result.im = norm * sin(arg);
		return result;
	}
