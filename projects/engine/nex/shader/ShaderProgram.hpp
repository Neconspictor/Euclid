 ...
 */
	...
	public:
		 ...
		 * @throws: ShaderInitException: If an error occurs.
		 */
		static std::unique_ptr<ShaderProgram> createComputeShader(const ShaderFilePath& computeFile, const std::vector<std::string>& defines = {});

		/**
		 * Creates a shader from a list of shader stages.
		 */
		static std::unique_ptr<ShaderProgram> create(const std::vector<std::unique_ptr<ShaderStage>>& stages);
