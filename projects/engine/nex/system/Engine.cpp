...
Engine::Engine() :
	m_logger("Engine"), m_systemLogLevel(Debug)
{
...
Engine::Engine(const Engine& other): m_logger(other.m_logger),
...
void Engine::add(SystemPtr system)
{
	...
	{
		LOG(m_logger, Warning) << "System already added: " << system->getName();
		return;
	}
...
Engine::SystemPtr Engine::get(const string& name) const
{
	...
	{
		LOG(m_logger, Warning) << "Cannot find System" << name;
		return nullptr;
	}
...
void Engine::init()
{
	...
	LOG(m_logger, Info) << "Loading configuration file...";
	if (!m_config.load(m_configFileName))
	{
		LOG(m_logger, Warning) << "Configuration file couldn't be read. Default values are used.";
	}
	else
	{
		LOG(m_logger, Info) << "Configuration file loaded.";
	}
	...
	catch (const EnumFormatException& e)
	{
		...
		LOG(m_logger, Error) << e.what();

		LOG(m_logger, Warning) << "Couldn't get log level from " << m_systemLogLevelStr << endl
	...
	LoggerManager::get()->setMinLogLevel(m_systemLogLevel);
	...
	LOG(m_logger, Info) << "root Directory = " << ::util::Globals::getRootDirectory();
...
void Engine::initSystems()
{
	for (auto it : m_systemMap)
	{
		...
		LOG(m_logger, Info) << "Initializing " << system->getName();
		system->init();
	}
}
...
void Engine::shutdownSystems()
{
	...
	{
		SystemPtr system = it.second;
		LOG(m_logger, Info) << "Shutting down" << system->getName();
		system->shutdown();
	}
}