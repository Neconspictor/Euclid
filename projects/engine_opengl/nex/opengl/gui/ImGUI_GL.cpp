...
namespace nex::gui
{
		...
		for (int n = 0; n < draw_data->CmdListsCount; n++)
		{
			...
			for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
			{
				...
				else
				{
					...
					auto* glTexture = (Texture2DGL*)texture->getImpl();
	...
	bool ImGUI_GL::createDeviceObjects()
	{
		...
		const GLchar* vertex_shader_with_version[2] = { "#version 420\n", vertex_shader };
		const GLchar* fragment_shader_with_version[2] = { "#version 420\n", fragment_shader };
