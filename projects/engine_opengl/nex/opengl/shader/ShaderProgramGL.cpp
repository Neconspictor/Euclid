#include <string>
#include <vector>
#include <nex/opengl/shader/ShaderProgramGL.hpp>
...
nex::ShaderProgram::~ShaderProgram() = default;

bool nex::ShaderProgram::isBound() const
{
...
nex::ShaderProgram::Impl::Impl(Impl&& other) noexcept : mDebugName(std::move(other.mDebugName)), 
...
nex::ShaderProgram::Impl& nex::ShaderProgram::Impl::operator=(Impl&& other) noexcept
{
...
int nex::ShaderProgram::Impl::getUniformBufferBindingPoint(const char * name) const
{
	int result;
...
bool nex::ShaderProgram::Impl::isBound() const
{
...
int nex::ShaderProgram::getUniformBufferBindingPoint(const char* name) const {
...
std::unique_ptr<nex::ShaderProgram> nex::ShaderProgram::create(
	...
	const std::vector<std::string>& defines)
{
	...
	{
		throw_with_trace(ShaderException("nex::ShaderProgram::create: No vertex or fragment shader specified!"));
	}
	...
	{
		if (! (useControlShader && useEvaluationShader))
			throw_with_trace(ShaderException("nex::ShaderProgram::create: Specified only one of the tesselation shader stages!"));
	}
...
std::unique_ptr<nex::ShaderProgram> nex::ShaderProgram::create(const std::vector<std::unique_ptr<ShaderStage>>& stages)
{
...
GLuint nex::ShaderProgram::Impl::createShaderProgram(const std::vector<std::unique_ptr<ShaderStage>>& stages)
{
	GLuint program;
	int infoLogLength;
	...
	if (result == GL_FALSE)
	{
		...
		throw_with_trace(ShaderException("Error: ShaderProgram::loadShaders(): Couldn't create shader program!"));
	}
